...
Response caching (§6.2) and adaptive concurrency backoff on sustained 429s are
seams left for P2 — this module calls `provider.generate` directly every time.
Retries already live in the provider layer (`providers/base.py`), so this
module never re-implements backoff.
"""
